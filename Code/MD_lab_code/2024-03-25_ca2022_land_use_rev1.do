...
local results_dir  =  "Z:\Resultados\LM2304-CA22-`date'\"
capture mkdir "`results_dir'"

*** Insumos
local location_22  =  "`ca22_dir'TRD_UBICACION_UP.dta"
...
*** Intermediarios
local loc_clean    =  "`ca22_cln_dir'location.dta"
...
****** LOCATION OF FARM UNIT
******
*2022*
******

use "`location_22'", clear
drop D_R

gen muncode = cve_ent+cve_mun
drop nom_ent nom_mun cve_mun
...
*** There are 255,183 missings on this merge from loc_clean, possibly forest/fisheries, no merge==1
merge m:1 id_ca22_up using "`loc_clean'", keep(3) nogen

**** DELETE THIS LATER
**** For now we don't have agebs nor adc indicators, include these so that we can write code with them
gen adc  = ""
gen ageb = ""
**** For now we don't have agebs nor adc indicators, include these so that we can write code with them
**** DELETE THIS LATER
...
/*
**** DONT HAVE THE AGEB/ADC information to do this yet
preserve
*** Collapse down and save at ADC level
collapse (sum) land_*t vol_*t q* num_terrenos, by(cve_ent muncode ageb adc name greenhouse)
...
collapse (sum) land_*t vol_*t q* num_terrenos, by(cve_ent muncode ageb name greenhouse)
...
collapse (sum) land_*t vol_*t q* num_terrenos, by(cve_ent muncode name greenhouse)

*** Keep only if not greenhouse
keep if greenhouse == 0 
drop greenhouse

keep if num_terrenos >= 3
gen share_irrig = land_irr_input/(land_irr_input+land_rf_input)
...
preserve
*** Collapse down and save at ADC level
collapse (sum) land_*t vol_*t q* num_terrenos, by(cve_ent muncode ageb adc type name greenhouse)
...
preserve
*** Collapse down and save at AGEB level
collapse (sum) land_*t vol_*t q* num_terrenos, by(cve_ent muncode ageb type name greenhouse)
...
preserve
*** Collapse down and save at MUN level
collapse (sum) land_*t vol_*t q* num_terrenos, by(cve_ent muncode type name greenhouse)
