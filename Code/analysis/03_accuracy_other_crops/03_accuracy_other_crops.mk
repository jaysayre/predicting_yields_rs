...
#   1_other_crops_adc_table.py   ADC-level other-crop table from the per-crop prediction parquets
#   2_other_crops_mun_agg.py     municipality-aggregated other-crop table (ex-ante weights)
# The per-crop parquets are trained by train/03_train_rf_gb/
# 4_gb_aef_hist_ensemble_other_crops.py (make train_other_crops).
...
$(PREDS_DIR)/adc_aef_hist_ens_preds_sorghum.parquet: $(TRAIN_DIR)/4_gb_aef_hist_ensemble_other_crops.py
	cd $(DATA_DIR) && $(ML_ENV) python3 $<

# Step 1 — ADC-level table: assemble ensemble rows
...
# Step 2 — municipality-aggregated accuracy (ex-ante agland weights)
