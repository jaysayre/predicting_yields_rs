...
Run:  ~/miniforge3/envs/geo_env/bin/python 17_rho_irrigation_bound.py
"""
...
r_hat   =  sd_pred/SD_Y_W        # dispersion ratio from the same run (was a hard-coded 0.655 until 2026-09-02)
print(f"midpoint point estimate: rho_hat = {rho_mid:.3f}  r_hat = {r_hat:.3f}  lambda_hat = {rho_mid/r_hat:.3f}")

out =  {"beta": round(beta, 2), "c_pred_irr": round(c, 3), "sd_irr": round(sd_irr, 3),
        "rho_midpoint": round(rho_mid, 3), "r_hat": round(r_hat, 3), "lambda_midpoint": round(rho_mid/r_hat, 3),
        ...
        "n_adc": int(len(ev))}
sp_ =  os.path.join(os.path.expanduser("~"), "Dropbox", "Projects", "Maize_prediction", "output")
