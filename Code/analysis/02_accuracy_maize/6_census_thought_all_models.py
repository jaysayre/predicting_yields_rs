...
(combined season) under three additive ex-post corrections:
  Raw              : no correction
  Corr. -> DGSIAP    : shift each municipality's predictions to the DGSIAP mun mean
...
(census) municipality yields rather than survey-based DGSIAP yields.
...
deployed lambda = 0.72 (public point estimate, Sec 3.6; was census-CV, same machinery as
within_mun_shrink_all_models.py), so the table also reports the shrink
estimates under each anchoring choice.

Models: NDVI (Landsat-derived); AEF mean, Agg-NN, AEF Hist, AEF Hist Ens.
(AEF-derived); plus the Agg-NN retrained on census municipal yields as a
...
# DGSIAP mun yield (combined, maize 2022)
...
MODELS = [  # (panel label, file, col)
    ...
    # ensemble-avg preds from the census-trained retrain (5_census_thought_experiment.py)
...
L = [r"\begin{table}[!htbp]", r"\centering",
     ...
     r"toward INEGI census municipality yields vs.\ DGSIAP municipality yields, for all models. "
     ...
     r"municipality-mean prediction by the deployed $\lambda = 0.72$ (Section~\ref{sec:shrink}). RMSE in t/ha.}",
