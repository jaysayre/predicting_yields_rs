...
2_gb_aef_hist_ensemble.py to Sorghum, Sugar, Wheat, and Avocados.
...
For each crop:
  ...
  3. Ensemble: w*bins + (1-w)*pct, w=0.5
  4. Evaluate vs INEGI 2022 census
  5. Apply additive ex-post correction

Persists per-crop ADC prediction parquets -- ensemble (pred_{crop},
pred_{crop}_corr, pred_{crop}_shrink) and the bins-only "AEF Hist" model
(pred_{crop}_bin, pred_{crop}_bin_corr); the paper table is assembled by
...
Usage:
  ...
  python3 4_gb_aef_hist_ensemble_other_crops.py
"""
...
# Crop -> dominant SIAP season (matches 1_rf_yield_prediction.py)
CROP_SEASONS = {
...
# Drop rows whose features are ENTIRELY absent (ported from 2_gb_aef_hist_ensemble.py,
...
for crop_name, season in CROP_SEASONS.items():
    ...
    # 2026-09-17: the bins-only prediction is the paper's "AEF Hist" model, so
    # persist it alongside the ensemble.
    adc_combo[f'pred_{crop_name}_bin'] = pred_bin

    # Ground truth
    gt_crop = ca[ca['name'] == crop_name][['adc', 'muncode', 'yield', 'land_input']].copy()
    df = gt_crop.merge(
        adc_combo[['adc', f'pred_{crop_name}', f'pred_{crop_name}_bin']],
        on='adc', how='left'
    )
    ...
    # Same additive correction applied to the bins-only ("AEF Hist") column
    bcol = f'pred_{crop_name}_bin'
    df['wQ_bin'] = df[bcol] * df['corr_w']
    df['wA_bin'] = df.apply(
        lambda x, c=bcol: x['corr_w'] if np.isfinite(x[c]) else 0, axis=1)
    mun_agg_b = df.groupby('muncode').agg({'wQ_bin': 'sum', 'wA_bin': 'sum'}).reset_index()
    mun_agg_b['pred_mun_avg'] = mun_agg_b['wQ_bin'] / mun_agg_b['wA_bin']
    mun_agg_b.loc[mun_agg_b['wA_bin'] == 0, 'pred_mun_avg'] = np.nan
    mun_agg_b = mun_agg_b.merge(siap_mun, on='muncode', how='left')
    mun_agg_b['diff_bin'] = mun_agg_b['pred_mun_avg'] - mun_agg_b['yield_siap']

    df = df.merge(mun_agg_b[['muncode', 'diff_bin']], on='muncode', how='left')
    bcorr_col = f'pred_{crop_name}_bin_corr'
    df[bcorr_col] = (df[bcol] - df['diff_bin']).clip(lower=0)
    df.loc[df[bcol].isna(), bcorr_col] = np.nan

    # Evaluate
    print(f"\n  {'Model':<45s} {'N':>8s} {'R2':>6s} {'Btw':>6s} {'Wtn':>7s} {'RMSE':>6s}")
    print(f"  {'-'*77}")

    m_bin  = eval_row(df, 'yield', bcol,      f'{crop_name} AEF Hist (bins only)')
    m_bcor = eval_row(df, 'yield', bcorr_col, f'{crop_name} AEF Hist Corr.')
    ...
    all_adc_results.append({'Crop': disp_name, 'Model': 'AEF Hist', **m_bin})
    all_adc_results.append({'Crop': disp_name, 'Model': 'AEF Hist Corr.', **m_bcor})
    ...
    df[['adc', 'muncode', pcol, corr_col, shr_col,
        bcol, bcorr_col]].to_parquet(_out_pq, index=False)
