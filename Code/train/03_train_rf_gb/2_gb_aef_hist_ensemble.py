...
Best config from sweep (2026-03-20):
  N=2 pixels per subsample, K=5 subsamples per mun-year, w=0.4  (census criterion)
DEPLOYED 2026-09-02: w=0.5, the public-data argmax (18_w_public_selection.py), applied to every ensemble.
...
Usage:
  ...
  python3 2_gb_aef_hist_ensemble.py
"""
...
W_BIN       =  0.5     # ensemble weight on bins model: public irrigation-projection argmax
                       # (analysis/02_accuracy_maize/18_w_public_selection.py). The census-based sweep
                       # of 2026-03-20 gave 0.4; within-R2 is flat on [0.35, 0.65] under either criterion.
...
# the P-V corrected R2. Same defect fixed in 4_accuracy_main_2022.py, where it was
...
# Component predictions (w-independent) -- inputs to the public w selection
comp_out  =  adc_combo[['adcid', 'year']].copy()
comp_out['muncode']   =  comp_out['adcid'].str[:5]
comp_out['pred_bin']  =  pred_bin
comp_out['pred_pct']  =  pred_pct
comp_path  =  os.path.join(pred_dir, f"adc_aef_hist_ens_components{TAG}.parquet")
comp_out.to_parquet(comp_path, index=False)
print(f"  {comp_path}  ({len(comp_out):,} rows)  [w-selection inputs]")

# Bins-only predictions: this IS the paper's "AEF Hist" model (2026-09-17), the
# 512-feature subsampled-histogram HistGB; the percentile model is only an
# ensemble component. Consumed by the analysis scripts in place of the retired
# 448-feature adc_aef_hist_gb_preds.parquet.
bins_out  =  comp_out[['adcid', 'muncode', 'year']].copy()
bins_out['yield_pred']  =  pred_bin
bins_path  =  os.path.join(pred_dir, f"adc_aef_hist_bins_gb_preds{TAG}.parquet")
bins_out.to_parquet(bins_path, index=False)
print(f"  {bins_path}  ({len(bins_out):,} rows)  [AEF Hist = bins-only model]")
