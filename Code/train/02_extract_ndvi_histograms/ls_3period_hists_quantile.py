"""
ls_3period_hists_quantile.py — Extract 3-period 2D NDVI histograms with quantile-based
bin edges from Google Earth Engine.

Same as ls_3period_hists.py but uses non-uniform bin edges derived from the empirical
NDVI distribution (stored in quantile_bin_edges_16.json). This concentrates resolution
in the NDVI range where crop variation is highest (~0.2–0.87).

Usage:
  python ls_3period_hists_quantile.py <level> <bins>
  python ls_3period_hists_quantile.py muni 16
  python ls_3period_hists_quantile.py adc 16
...
def assign_bin_quantile(img, edges):
    """Assign each pixel to a bin index using non-uniform edges.

    Uses a cascade of .where() calls. For 16 bins this is 15 comparisons
    per pixel — negligible overhead in the GEE computation graph.
    """
    n_bins  =  len(edges) - 1
    bin_img =  ee.Image.constant(0)
    for i in range(1, n_bins):
        bin_img =  bin_img.where(img.gte(edges[i]), i)
    return bin_img
...
def compute_3period_hist(feature):
    ...
    # Bin each period using quantile edges
    bin_p1 =  assign_bin_quantile(ndvi_p1.select('ndvi'), BIN_EDGES)
    bin_p2 =  assign_bin_quantile(ndvi_p2.select('ndvi'), BIN_EDGES)
    bin_p3 =  assign_bin_quantile(ndvi_p3.select('ndvi'), BIN_EDGES)
...
# ── Export folder ────────────────────────────────────────
folder =  f"{level}_3period_hists_{bins}bins_quantile"
...
def submit_batch(batch_df, state, batch_idx, year, level, id_col, folder, bins):
    ...
    desc =  f"3period_qhist_{level}_{state}_{batch_idx}_{year}"
