"""
ee_alpha_earth_histogram_2022.py
================================
Fill the 2022 gap: extract distributional AEF features for year 2022 only,
for all states EXCEPT state 20 (Oaxaca, already done in POC).

The all-years script (ee_alpha_earth_histogram_allyears.py) extracted
2017-2021, 2023-2024 but skipped 2022. The POC script only extracted
2022 for Oaxaca. This script fills the remaining 31 states for 2022.

Submits both ADC-level and municipality-level tasks.

Usage:
  conda activate ML_env
  python3 ee_alpha_earth_histogram_2022.py
  python3 ee_alpha_earth_histogram_2022.py --skip_mun          # ADCs only
  python3 ee_alpha_earth_histogram_2022.py --start_state 15    # resume from state 15
...
# ============================================================
YEARS          =  [2022]
SKIP_STATES    =  ['20']  # Oaxaca already extracted in POC
ADC_BATCH      =  2000
ADC_BATCH_OAX  =  500     # (unused since we skip state 20)
SCALE          =  10
...
def main():
    ...
    print(f"Years: {YEARS}")
    print(f"Skip states: {SKIP_STATES}")
    print(f"ADC batch size: {ADC_BATCH}")
    ...
    print(f"  {len(adcs)} ADCs across {adcs['est'].nunique()} states (excl {SKIP_STATES})")
    ...
    muns =  muns[~muns['CVE_ENT'].isin(SKIP_STATES)]
    ...
    if args.skip_mun:
        ...
        print("\n" + "=" * 60)
        print("  Submitting MUNICIPALITY-level tasks (2022)")
        print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  Submitting ADC-level tasks (2022)")
    print("=" * 60)
    ...
    print(f"{'='*60}")
    print(f"\nAfter tasks complete, download CSVs and re-run:")
    print(f"  python3 consolidate_hist_parquet.py")
