...
embeddings and a DGSIAP Spring-Summer maize label, 2017--2024).
...
3_train_holdout_validation_models.py with the held-out municipalities excluded
...
batches). Its holdout model comes from 3_train_holdout_validation_models.py
...
using DGSIAP agricultural-land area weights; NDVI predicts at the
municipality level directly. The Agg-NN row uses the inner-ES model
(adc_agg_nn_preds_maize_phase2_inner_es.parquet, from 1_agg_constrained_nn.py
...
Run:  source /usr/local/anaconda3/etc/profile.d/conda.sh && conda activate mpc_env
      python3 4_validation_mun_level.py
"""
import os
import numpy as np
import pandas as pd
...
# ── DGSIAP Spring-Summer maize labels (2017+) ─────────────
...
# All *_holdout_* files come from 3_train_holdout_validation_models.py (models
...
# 2026-09-17: the "AEF Hist" row is now the BINS-ONLY model (64 dims x 8
# fixed-width histogram-bin shares = 512 features, K=5 subsampled draws of N=2
# pixels per dim) -- i.e. the bins component of the AEF Hist Ensemble, written
# by 3_train_holdout_validation_models.py train_ensemble(). The old 448-feature
# percentile/SD/mean model (adc_aef_hist_gb_holdout_preds.parquet) is no longer
# reported on its own.
MUN_LEVEL = {"NDVI"}
MODELS = [
    ...
    ("AEF-derived features",     "AEF Hist",       "adc_aef_hist_bins_gb_holdout_preds.parquet",  "yield_pred"),
...
# municipalities and to mun-years with a DGSIAP label
common = None
