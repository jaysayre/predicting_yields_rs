"""
3_train_holdout_validation_models.py — standardized inputs for Table 1
...
were trained at the mun level on ALL municipalities (1_rf_yield_prediction.py
refits on everything before predicting ADCs; 2_gb_aef_hist_ensemble.py never
...
1_agg_constrained_nn.py uses, reproduced exactly as in 4_validation_mun_level.py),
...
predictions cover all years 2017-2024. 4_validation_mun_level.py then scores all
...
Models / outputs (all to Data/predictions/):
  ...
  AEF Hist       adc_aef_hist_gb_holdout_preds.parquet              (ADC-level, superseded)
  AEF Hist (bins) adc_aef_hist_bins_gb_holdout_preds.parquet        (ADC-level)
...
The "AEF Hist" row in Table 1 is, as of 2026-09-17, the BINS-ONLY model: the
HistGradientBoosting model trained on 64 dims x 8 fixed-width histogram-bin
shares (512 features) with K=5 subsampled draws of N=2 pixels per dimension --
i.e. exactly the bins component of the AEF Hist Ensemble. It is written by
train_ensemble() alongside the ensemble output. The old 448-feature
percentile/SD/mean model (train_aef_hist) is no longer reported on its own.

Usage:
  source /usr/local/anaconda3/etc/profile.d/conda.sh && conda activate mpc_env
  python3 3_train_holdout_validation_models.py
  python3 3_train_holdout_validation_models.py --only ensemble   # section E only
"""
import os, sys, time
import numpy  as np
import pandas as pd
...
N_PIX, K_SAMP, W_BIN  =  2, 5, 0.5                                  # gb_aef_hist_ensemble config
...
# (identical to 4_validation_mun_level.py / 1_agg_constrained_nn.py)
...
def train_aef_mean():
    ...
    # 2026-09-03: means-alone RF (matches 1_rf_yield_prediction.py IMPROVED=False);
    # the GB variant fed CVE_ENT/CVE_MUN/year as features, which act like a
    # municipality lookup — dropped by coauthor decision.
    ...
    best  =  (None, -np.inf)
    for depth in [1, 5, 10, 20, 50, 100, None]:
        m  =  RandomForestRegressor(max_depth=depth, n_estimators=100,
                                    random_state=SEED, n_jobs=-1)
        m.fit(X_tr[itr], y_tr[itr])
        r2  =  calc_r2(y_tr[iva], m.predict(X_tr[iva]))
        print(f"  RF max_depth={str(depth):>4s}  inner R2={r2:.4f}")
        if r2 > best[1]:
            best  =  (depth, r2)
    m  =  RandomForestRegressor(max_depth=best[0], n_estimators=100,
                                random_state=SEED, n_jobs=-1)
    m.fit(X_tr, y_tr)
    print(f"  cfg: max_depth={best[0]}")
...
def subsample_bins(train_df, bin_cols, K, N, seed=42):
    """Multinomial subsampling of mun bin distributions (as 2_gb_aef_hist_ensemble.py)."""
...
def train_ensemble():
    ...
    for yr in YEARS:
        ...
        o['yield_pred'] =  pb                                # bins-only component
    ...
    # bins-only predictions = the "AEF Hist" row of Table 1 (2026-09-17)
    bpath  =  os.path.join(pred_dir, "adc_aef_hist_bins_gb_holdout_preds.parquet")
    out[['adcid', 'muncode', 'year', 'yield_pred']].to_parquet(bpath, index=False)
    print(f"  saved -> {os.path.basename(bpath)}  ({len(out):,} rows)")
...
if __name__ == '__main__':
    ...
    if 'masked' in sys.argv:
        ...
        sys.exit(0)
    if 'aef_mean' in sys.argv:   # re-run only the AEF mean row
        train_aef_mean()
        print(f"\nTotal runtime: {(time.time()-t0)/60:.1f} min")
        sys.exit(0)
    if '--only' in sys.argv:     # run a single section, e.g. --only ensemble
        which  =  sys.argv[sys.argv.index('--only') + 1]
        {'ndvi_h3_fixed':    lambda: train_ndvi('h3_fixed'),
         'ndvi_h3_quantile': lambda: train_ndvi('h3_quantile'),
         'ndvi':             train_ndvi_masked,
         'aef_mean':         train_aef_mean,
         'aef_hist':         train_aef_hist,
         'ensemble':         train_ensemble}[which]()
        print(f"\nTotal runtime: {(time.time()-t0)/60:.1f} min")
        sys.exit(0)
    train_ndvi('h3_fixed')
